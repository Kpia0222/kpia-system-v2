...
import { LevelAsteroids } from "@/components/canvas/environments/LevelAsteroids";

import { UI_STRINGS } from "@/config/ui-strings";
import { SYSTEM_LOGS, getRandomLogParam } from "@/config/lore-content";

// --- Mock Data Generator for Right Panel ---
const PARAM_NAMES = SYSTEM_LOGS.PARAMETERS;

const generateRandomLine = () => {
    const isError = Math.random() < 0.05;
    const isScanning = Math.random() < 0.05;

    if (isError) return { type: "status", text: SYSTEM_LOGS.STATUS_ERROR, color: "#00ff00" };
    if (isScanning) return { type: "status", text: SYSTEM_LOGS.STATUS_SCANNING, color: "#ff8800" };
        ...
        <div className="w-full h-full relative pointer-events-none md:p-12 p-4 flex justify-between items-center text-white font-mono overflow-hidden">
            ...
            >
                ...
                <div className="relative z-10 skew-x-[12deg] flex flex-col items-center">
                    <h2 className="text-xl tracking-[0.5em] text-white/70 mb-2">{UI_STRINGS.DNA_UI.LEVEL_LABEL}</h2>
                    ...
                    <div className="mt-8 text-xs tracking-widest text-[#ff8800]">
                        {UI_STRINGS.DNA_UI.STABILITY_LABEL}
                    </div>
                </div>
            </motion.div>
            ...
            >
                ...
                <div className="relative z-10 px-6 py-4 border-b border-[#ff8800]/30 flex justify-between items-center bg-black/20">
                    <span className="text-sm tracking-widest text-[#ff8800]">{UI_STRINGS.DNA_UI.LOG_HEADER}</span>
