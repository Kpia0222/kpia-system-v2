...
const Satellite = ({ distance, speed, size, color, galaxyType }: SatelliteProps) => {
    ...
    return (
        <>
            ...
            <OrbitTrail currentAngle={angle} radius={distance} thickness={0.005} opacity={0.08} color="#888888" />
...
const Planet = ({ distance, speed, size, color, galaxyType }: PlanetProps) => {
    ...
    return (
        <group rotation={planetInclination}>
            ...
            <OrbitTrail currentAngle={angle} radius={distance} thickness={0.02} opacity={0.12} color={color} />
            ...
            <group ref={groupRef}>
                ...
                {satellites.map((s, i) => (
                    <group key={`sat-orbit-${i}`} rotation={s.inclination}>
                        ...
                        <Satellite distance={s.dist} speed={s.speed} size={s.size} color={s.color} galaxyType={galaxyType} />
                    </group>
                ))}
...
const SolarSystem = ({ index, distance, speed, orbitAngle, galaxyType }: SolarSystemProps) => {
    const groupRef = useRef<THREE.Group>(null);
    ...
    return (
        <>
            ...
            <OrbitTrail currentAngle={angle} radius={distance} thickness={0.08} opacity={0.2} color="#ffffff" />
            ...
            <group ref={groupRef}>
                ...
                <mesh ref={starRef}>
                    ...
                    <meshBasicMaterial color="#fff" wireframe={isWestern} />
                </mesh>
                <pointLight distance={15} intensity={1} color="#fff8e0" decay={2} />

                {/* Planets */}
                {planets.map((p, i) => (
                    <Planet key={`planet-${i}`} distance={p.dist} speed={p.speed} size={p.size} color={p.color} galaxyType={galaxyType} />
                ))}
            </group>
        </>
    );
